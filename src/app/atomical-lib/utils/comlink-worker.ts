...
export const mineBitwork = async (message: WorkerInput) => {
    const {
        copiedData,
        seqStart,
        seqEnd,
    ...
    let updatedBaseCommit: { scriptP2TR: any; hashLockP2TR: any; hashscript: any } =
    ...
    let finalCopyData: any, finalPrelimTx: any;
    ...
    do {
        ...
        if (sequence > seqEnd) {
            ...
            const newBaseCommit: { scriptP2TR: any; hashLockP2TR: any; hashscript: any } =
                workerPrepareCommitRevealConfig(
                    workerOptions.opType,
                    fundingKeypair,
                    atomPayload
...
export interface MineWorker {
  mineBitwork: typeof mineBitwork
}

const mineWorker: MineWorker = {
  mineBitwork
}
